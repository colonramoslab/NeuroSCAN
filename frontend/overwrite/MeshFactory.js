...
export default class MeshFactory {
  ...
  async loadThreeGLTFModelFromNode (node) {
    ...
    const gltfData = await this.modelParser(loader, this.resolveGltfPayload(node.gltf));
  ...
  resolveGltfPayload (input) {
    if (input == null) return input;
    if (input instanceof ArrayBuffer) return input;
    if (ArrayBuffer.isView && ArrayBuffer.isView(input)) return input.buffer;
  ...
  async modelParser (loader, data, resourcePath) {
    let results = await new Promise((resolve, reject) => {
      loader.parse(data, resourcePath || '', data => {
        return resolve(data);
      }, reject);
    });
    return results;
